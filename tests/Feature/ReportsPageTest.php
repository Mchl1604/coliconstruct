...
class ReportsPageTest extends TestCase
{
    ...
    public function test_the_page_renders_both_tabs(): void
    {
        ...
        // Generating a system report now produces a preview to read, not a
        // file to open - Print and PDF are chosen from that preview.
    ...
    public function test_the_project_breakdown_counts_derived_statuses(): void
    {
        ...
        $this->assertSame(1, $slices['Needs Rescheduling']);
    ...
    /**
     * Generate Report produces something to read, not something to download.
     * Every report type comes back as a document the page can show.
     */
    public function test_it_previews_every_report_type_as_a_document(): void
    {
        $tech = $this->technician('Ana Mendoza');
        $project = $this->project('Some Project', 'ongoing', [$tech], 400000);
        $this->schedule($project, $this->dateThisYear('08-02'), $this->dateThisYear('08-06'));

        foreach (ReportController::EXPORT_TYPES as $type => $label) {
            $response = $this->postJson(route('super-admin.reports.preview'), $this->exportPayload([
                'report_type' => $type,
            ]));

            $response->assertOk();
            $response->assertJsonPath('title', $label);

            $html = $response->json('html');

            $this->assertStringContainsString('class="report-doc"', $html, "{$type} did not render a document.");
            $this->assertStringContainsString($label, $html, "{$type} did not print its own title.");
        }
    }

    /**
     * The sign-off is part of the report, and it names whoever ran it - not a
     * name the page chose and sent along.
     */
    public function test_the_preview_is_signed_off_by_the_user_who_generated_it(): void
    {
        $this->project('Some Project');

        $html = $this->postJson(
            route('super-admin.reports.preview'),
            $this->exportPayload()
        )->json('html');

        $this->assertStringContainsString('Generated By', $html);
        $this->assertStringContainsString('Test Super Admin', $html);
        $this->assertStringContainsString('signature-line', $html);
    }

    /**
     * Statuses are words on a report, never fills. A document that is
     * photocopied in black and white cannot keep meaning in a colour.
     */
    public function test_report_statuses_are_printed_as_plain_text(): void
    {
        $this->project('Some Project', 'ongoing');
    ...
    /**
     * The PDF is the same document, so it carries the same sign-off and the
     * same plain-text statuses. Checked on the template rather than through
     * dompdf: what reaches the page is what is being asserted, and rendering
     * a year of work to PDF to read four strings out of it is a slow way to
     * ask a simple question.
     */
    public function test_the_pdf_carries_the_sign_off_and_no_status_colours(): void
    {
        $this->project('Some Project', 'ongoing');

        $period = $this->monthOf(CarbonImmutable::today()->toDateString());
        $report = app(SystemReportService::class)->exportReport('project', $period, []);

        $html = view('super-admin.reports-pdf', [
            'report' => $report,
            'reportTitle' => $report['title'],
            'period' => $period,
            'appliedFilters' => ['Project Status' => 'All Statuses'],
    ...
    /**
     * The preview and the PDF are two renderings of one build, so a request
     * the export refuses is refused here as well.
     */
    public function test_the_preview_refuses_the_filters_the_export_refuses(): void
    {
        $response = $this->postJson(route('super-admin.reports.preview'), $this->exportPayload([
